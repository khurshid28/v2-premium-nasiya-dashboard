...
const Admins = (): JSX.Element => {
  ...
  const merchants = React.useMemo(() => {
    ...
    data.forEach((d) => {
      if (d.merchant_id && d.merchant?.name) {
        set.set(d.merchant_id, d.merchant.name);
      }
    });
  ...
  const filteredData = React.useMemo(() => {
    ...
    if (merchantFilter !== "all") {
      filtered = filtered.filter(item => item.merchant_id === Number(merchantFilter));
    }
    
    return filtered;
  }, [data, search, merchantFilter]);
  ...
  return (
    <div>
      <div className="mb-4">
        <h2 className="text-xl sm:text-2xl font-semibold text-navy-700 dark:text-white">Adminlar</h2>
      </div>
      ...
      <div className="flex flex-col gap-3 mb-4">
        <div className="relative w-full sm:w-80 lg:w-96">
          <input
            ...
            placeholder="Ism, telefon yoki merchant bo'yicha qidirish"
          />
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button
            onClick={async () => {
              ...
              exportSingleTable({ rows, title: "Adminlar", dateLabel: "Barcha sanalar" });
            }}
        ...
        <table className="w-full table-auto min-w-[600px]">
          <thead className="bg-gray-50 dark:bg-navy-800 text-left text-xs sm:text-sm text-gray-600 dark:text-gray-300">
            <tr>
              ...
              <th className="px-3 sm:px-6 py-3 sm:py-4 hidden md:table-cell">Merchant</th>
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan={6} className="px-4 py-6 text-center text-sm text-gray-500 dark:text-gray-400">
                  {search || merchantFilter !== "all" ? "Qidiruv bo'yicha adminlar topilmadi" : "Adminlar mavjud emas"}
                </td>
              </tr>
            ) : pageData.map((row) => (
              <tr
                ...
                onClick={async () => {
                  try {
                    ...
                    const fullAdmin = await api.getAdmin(row.id);
                    setSelected(fullAdmin);
                    setOpen(true);
                  } catch (err) {
                    console.error("Error fetching admin details:", err);
                    setToastType("error");
                    setToastMessage("Admin tafsilotlarini yuklashda xatolik yuz berdi");
                    setToastOpen(true);
                    setSelected(row);
                    setOpen(true);
                  } finally {
              ...
              >
                ...
                <td className="px-3 sm:px-6 py-2 sm:py-3 text-xs sm:text-sm hidden md:table-cell">{row.merchant?.name ?? "-"}</td>
      ...
      <DetailModal
        title={selected ? `Admin: ${selected.fullname}` : "Admin tafsilotlari"}
        isOpen={open}
        onClose={() => {
          setOpen(false);
          setSelected(null);
      ...
      <PasswordModal
        ...
        onSave={async (payload) => {
          ...
          if (selected && selected.id) {
            await api.updateAdminPassword(selected.id, { password: payload });
          }
          setPasswordOpen(false);
          setSelected(null);
        }}
      ...
      <EditModal
        ...
        onSave={async (payload) => {
          if (location.pathname.startsWith('/demo')) {
            ...
            setEditInitial(null);
            setToastType('success');
            setToastMessage(editInitial ? 'Admin tahrirlandi (Demo)' : 'Admin qo\'shildi (Demo)');
            setToastOpen(true);
            return;
          }
          if (editInitial && editInitial.id) {
            await api.updateAdmin(editInitial.id, payload);
          } else {
